...
public class TextDocumentLoader implements DocumentLoader {
    ...
    public String extractText(InputStream inputStream, String filename) {
        log.info("Extracting text from TXT document: {}", filename);
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
            return reader.lines().collect(Collectors.joining("\n")).trim();
        } catch (Exception e) {
            log.error("Failed to parse TXT {}: {}", filename, e.getMessage());
            throw new RagException("Error reading Text file: " + filename, e);
        }
    }
}
